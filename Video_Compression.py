# -*- coding: utf-8 -*-
"""
Created on Wed May  3 10:32:38 2023
...
def find_downsampled():
  all = []
  for root, dirs, files in os.walk("/home/genzel/Desktop/ffmpeg"):
      for file in files:
          ...
          if file.endswith(".avi"):
            ...
            if 'downsampled' not in file:
              ...
              if already_in is False:
                  ...
                  os.system("ffmpeg -hwaccel cuda -i {old_file} -c:v libx264 -c:a copy {new_file}".format(old_file=old_file, new_file=new_file))
                  finished += 1
